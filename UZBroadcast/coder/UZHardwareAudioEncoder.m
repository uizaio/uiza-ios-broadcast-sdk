...
@interface UZHardwareAudioEncoder (){
...
@property (nonatomic, strong) UZAudioConfiguration *configuration;
@property (nonatomic, weak) id<UZAudioEncodingDelegate> aacDeleage;

@end

@implementation UZHardwareAudioEncoder
...
- (void)encodeBuffer:(char*)buf timeStamp:(uint64_t)timeStamp{
    ...
    UZAudioFrame *audioFrame = [UZAudioFrame new];
...
- (BOOL)createAudioConvert { // Initialize a transcoder based on input samples
    if (m_converter != nil) {
        return TRUE;
    }
    ...
    AudioStreamBasicDescription outputFormat; // the output audio format
    memset(&outputFormat, 0, sizeof(outputFormat));
    outputFormat.mSampleRate = inputFormat.mSampleRate; // Consistent sampling rate
    outputFormat.mFormatID = kAudioFormatMPEG4AAC;   // AAC encoding kAudioFormatMPEG4AAC kAudioFormatMPEG4AAC_HE_V2
    outputFormat.mChannelsPerFrame = (UInt32)_configuration.numberOfChannels;;
    outputFormat.mFramesPerPacket = 1024;                     // AAC frame is 1024 bytes
    ...
    UInt32 outputBitrate = (unsigned int)_configuration.audioBitrate;
...
OSStatus inputDataProc(AudioConverterRef inConverter, UInt32 *ioNumberDataPackets, AudioBufferList *ioData, AudioStreamPacketDescription * *outDataPacketDescription, void *inUserData) { /// <span style="font-family: Arial, Helvetica, sans-serif;">AudioConverterFillComplexBuffer during the encoding process, this function will be required to fill in the input data, which is the original PCM data</span>
