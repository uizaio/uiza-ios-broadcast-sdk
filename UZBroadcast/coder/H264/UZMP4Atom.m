...
@implementation UZMP4Atom

@synthesize type = _type;
@synthesize length = _length;

+ (UZMP4Atom *)atomAt:(int64_t)offset size:(int)length type:(OSType)fourcc inFile:(NSFileHandle *)handle {
    UZMP4Atom *atom = [UZMP4Atom alloc];
...
- (UZMP4Atom *)nextChild {
    if (_nextChild <= (_length - 8)) {
        ...
        _nextChild += len;
        len -= cHeader;
        return [UZMP4Atom atomAt:offset+_offset size:(int)len type:fourcc inFile:_file];
    }
    return nil;
}

- (UZMP4Atom *)childOfType:(OSType)fourcc startAt:(int64_t)offset {
