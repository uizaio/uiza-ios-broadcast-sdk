...
@interface UZAVEncoder ()

{
    // initial writer, used to obtain SPS/PPS from header
    UZVideoEncoder *_headerWriter;

    // main encoder/writer
    UZVideoEncoder *_writer;
...
@implementation UZAVEncoder

@synthesize bitspersecond = _bitspersecond;

+ (UZAVEncoder *)encoderForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate {
    UZAVEncoder *enc = [UZAVEncoder alloc];
...
- (void)initForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate {
    ...
    _headerWriter = [UZVideoEncoder encoderForPath:path Height:height andWidth:width bitrate:(int)self.bitrate];
    _times = [NSMutableArray arrayWithCapacity:10];

    // swap between 3 filenames
    _currentFile = 1;
    _writer = [UZVideoEncoder encoderForPath:[self makeFilename] Height:height andWidth:width bitrate:(int)self.bitrate];
...
- (BOOL)parseParams:(NSString *)path {
    ...
    UZMP4Atom *movie = [UZMP4Atom atomAt:0 size:(int)s.st_size type:(OSType)('file') inFile:file];
    UZMP4Atom *moov = [movie childOfType:(OSType)('moov') startAt:0];
    UZMP4Atom *trak = nil;
    if (moov != nil) {
        for (;; ) {
            trak = [moov nextChild];
            ...
            if (trak.type == (OSType)('trak')) {
                UZMP4Atom *tkhd = [trak childOfType:(OSType)('tkhd') startAt:0];
    ...
    UZMP4Atom *stsd = nil;
    if (trak != nil) {
        UZMP4Atom *media = [trak childOfType:(OSType)('mdia') startAt:0];
        if (media != nil) {
            UZMP4Atom *minf = [media childOfType:(OSType)('minf') startAt:0];
            if (minf != nil) {
                UZMP4Atom *stbl = [minf childOfType:(OSType)('stbl') startAt:0];
                if (stbl != nil) {
    ...
    if (stsd != nil) {
        UZMP4Atom *avc1 = [stsd childOfType:(OSType)('avc1') startAt:8];
        if (avc1 != nil) {
            UZMP4Atom *esd = [avc1 childOfType:(OSType)('avcC') startAt:78];
            if (esd != nil) {
    ...
    @synchronized(self){
        ...
        if (!_swapping) {
            ...
            if (st.st_size > OUTPUT_FILE_SWITCH_POINT || self.bitrateChanged) {
                ...
                _swapping = YES;
                UZVideoEncoder *oldVideo = _writer;
                ...
                _writer = [UZVideoEncoder encoderForPath:[self makeFilename] Height:_height andWidth:_width bitrate:(int)self.bitrate];
...
- (void)onNALU:(NSData *)nalu {
    ...
    if (_pendingNALU) {
        UZNALUnit nal(pNal, (int)[nalu length]);
