...
bool
UZNALUnit::GetStartCode(const BYTE *& pBegin, const BYTE *& pStart, int& cRemain){
...
bool
UZNALUnit::Parse(const BYTE *pBuffer, int cSpace, int LengthSize, bool bEnd){
...
void
ScalingList(int size, UZNALUnit *pnalu){
    long lastScale = 8;
    long nextScale = 8;
...
bool
UZSeqParamSet::Parse(UZNALUnit *pnalu){
    if (pnalu->Type() != UZNALUnit::NAL_Sequence_Params) {
        return false;
    }
...
// --- slice header --------------------
bool
UZSliceHeader::Parse(UZNALUnit *pnalu){
    switch (pnalu->Type()) {
    case UZNALUnit::NAL_IDR_Slice:
    case UZNALUnit::NAL_Slice:
    case UZNALUnit::NAL_PartitionA:
...
UZSEIMessage::UZSEIMessage(UZNALUnit *pnalu){
    m_pnalu = pnalu;
...
UZavcCHeader::UZavcCHeader(const BYTE *header, int cBytes){
    if (cBytes < 8) {
        return;
    }
