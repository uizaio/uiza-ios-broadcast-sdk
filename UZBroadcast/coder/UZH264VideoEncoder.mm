...
@interface UZH264VideoEncoder() {
    FILE *fp;
    NSInteger frameCount;
    BOOL enabledWriteVideoFile;
}
@property (nonatomic, strong) UZVideoConfiguration *configuration;
@property (nonatomic, weak) id<UZVideoEncodingDelegate> h264Delegate;
@property (nonatomic) BOOL isBackGround;
@property (nonatomic) NSInteger currentVideoBitRate;
@property (nonatomic, strong) dispatch_queue_t sendQueue;

@property (nonatomic, strong) UZAVEncoder *encoder;
...
@implementation UZH264VideoEncoder
...
- (void)initCompressionSession{
    ...
    _encoder = [UZAVEncoder encoderForHeight:(int)_configuration.videoSize.height andWidth:(int)_configuration.videoSize.width bitrate:(int)_configuration.videoBitRate];
...
- (void) generateSPSandPPS {
    ...
    UZavcCHeader avcC((const BYTE*)[config bytes], (int)[config length]);
    UZSeqParamSet seqParams;
...
- (void)setDelegate:(id<UZVideoEncodingDelegate>)delegate{
...
- (void)writeVideoFrames:(NSArray*)frames pts:(CMTime)pts {
    ...
    for (NSData *data in totalFrames) {
        ...
        UZVideoFrame *videoFrame = [UZVideoFrame new];
