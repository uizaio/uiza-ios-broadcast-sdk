...
static const NSInteger RetryTimesBreaken = 5;  ///<  Reconnect 1 minute, 3 seconds, 20 times in total
...
static const AVal av_SDKVersion = AVC("UZBroadcast 1.2.4");
SAVC(onMetaData);
SAVC(duration);
SAVC(width);
SAVC(height);
...
@interface UZStreamRTMPSocket ()<UZStreamingBufferDelegate>
{
    PILI_RTMP *_rtmp;
}
...
- (void)setDelegate:(id<UZStreamSocketDelegate>)delegate {
    _delegate = delegate;
}
    ...
    __weak typeof(self) _self = self;
     dispatch_async(self.rtmpSendQueue, ^{
        if (!_self.isSending && _self.buffer.list.count > 0) {
            ...
            // Call the send interface
            UZFrame *frame = [_self.buffer popFirstObject];
            if ([frame isKindOfClass:[UZVideoFrame class]]) {
                if (!_self.sendVideoHead) {
                    _self.sendVideoHead = YES;
                    if(!((UZVideoFrame*)frame).sps || !((UZVideoFrame*)frame).pps){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendVideoHeader:(UZVideoFrame *)frame];
                } else {
                    [_self sendVideo:(UZVideoFrame *)frame];
                }
            } else {
                if (!_self.sendAudioHead) {
                    _self.sendAudioHead = YES;
                    if(!((UZAudioFrame*)frame).audioInfo){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendAudioHeader:(UZAudioFrame *)frame];
                } else {
            ...
            if (_self.debugInfo.elapsedMilli < 1000) {
                ...
                if ([frame isKindOfClass:[UZAudioFrame class]]) {
            ...
            /// Modify sending status
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                /// Here only to avoid calling the sendFrame method cyclically
                _self.isSending = NO;
            });
...
- (void)sendVideoHeader:(UZVideoFrame *)videoFrame {
...
- (void)sendVideo:(UZVideoFrame *)frame {
...
- (void)sendAudioHeader:(UZAudioFrame *)audioFrame {
    ...
    body[0] = 0xAF;
    body[1] = 0x00;
    memcpy(&body[2], audioFrame.audioInfo.bytes, audioFrame.audioInfo.length);  /*spec_buf is the AAC sequence header data*/
    [self sendPacket:RTMP_PACKET_TYPE_AUDIO data:body size:rtmpLength nTimestamp:0];
    free(body);
}

- (void)sendAudio:(UZFrame *)frame {
...
void RTMPErrorCallback(RTMPError *error, void *userData) {
    UZStreamRTMPSocket *socket = (__bridge UZStreamRTMPSocket *)userData;
    if (error->code < 0) {
        [socket reconnect];
    }
}
...
- (UZStreamingBuffer *)buffer {
    if (!_buffer) {
        _buffer = [[UZStreamingBuffer alloc] init];
